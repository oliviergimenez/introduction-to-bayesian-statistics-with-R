...
```{r hist-b0-reel-brms, fig.cap = "Distribution a posteriori de l'intercept moyen (échelle réelle). La ligne rouge indique la vraie valeur (0).", message = FALSE, warning = FALSE, dpi = 600}
tibble(b0 = bzero) %>%
  ggplot(aes(x = b0)) +
...
```{r hist-b1-reel-brms, fig.cap = "Distribution a posteriori de l'effet de la température (échelle réelle). La ligne rouge indique la vraie valeur (0.2).", message = FALSE, warning = FALSE, dpi = 600}
tibble(b1 = bun) %>%
  ggplot(aes(x = b1)) +
  geom_histogram(color = "white", fill = "skyblue", bins = 30) +
