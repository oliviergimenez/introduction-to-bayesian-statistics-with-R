...
```{r donnees-simulees, echo = FALSE, fig.cap = "Simulated data (n = 100) according to the model \\(y_i = \\beta_0 + \\beta_1 x_i + \\varepsilon_i\\), with \\(\\beta_0 = 0.1\\), \\(\\beta_1 = 1\\) and \\(\\sigma = 1\\). The red line corresponds to the regression line.", dpi = 600}
...
```{r fig-posterior-regression, echo = TRUE, fig.cap = "Histograms of the posterior distributions (left column) and traces (right column) of the linear regression parameters. In the histograms, the x-axis represents the possible values of the estimated parameter (intercept, slope, or standard deviation) and the y-axis corresponds to their frequency in the posterior sample. In the trace plots, the x-axis indicates the MCMC iteration number, while the y-axis represents the simulated value of the parameter at each iteration.", dpi = 600}
plot(lm.brms)
```
...
```{r fig-prior-regression-vague, echo = TRUE, fig.cap = "Simulation of regression lines drawn from the prior distributions. Each line corresponds to a draw of the parameters: intercept and slope ~ N(0, 100).", dpi = 600}
...
```{r fig-prior-regression, echo = FALSE, fig.cap = "Simulation of regression lines drawn from the prior distributions. Each line corresponds to a draw of the parameters: intercept and slope ~ N(0, 1.5).", dpi = 600}
...
df_long <- pivot_longer(df, cols = -sigma, names_to = "Distribution", values_to = "Density")

# plot
ggplot(df_long, aes(x = sigma, y = Density, color = Distribution)) +
  geom_line(size = 1.2) +
  labs(
    title = "",
...
n <- 100 # number of obs
x <- rnorm(n = n, mean = 0, sd = 1) # simulated x covarite
epsilon <- rnorm(n, mean = 0, sd = sigma) # errors
y <- beta0 + beta1 * x + epsilon # add errors to linear relationship
data <- data.frame(y = y, x = x)
dat <- list(x = x, y = y, n = n) # data
inits <- list(list(beta0 = -0.5, beta1 = -0.5, sigma = 0.1), # inits chain 1
              list(beta0 = 0, beta1 = 0, sigma = 1), # inits chain 2
...
```{r echo = FALSE}
# True values used in simulations
true_beta0 <- 0.1
true_beta1 <- 1
# ------------------------------------------------------------
# standard model lm()
...
results_df <- data.frame(
  ...
  Lower = c(
    confint(lm_fit)[1,1], confint(lm_fit)[2,1],            # lm: IC 95% 
    brms_estimates[, "l-95% CI"],                             # brms: IC 95% 
...
```{r brms-fit-plot, echo=TRUE, fig.cap="Linear model fit with brms. The blue line is the estimated regression, obtained by setting the intercept and slope to their posterior means, surrounded by its 95% credible interval.", dpi = 600}
...
```{r nimble-fit-plot, echo=TRUE, fig.cap="Linear model fit with NIMBLE. The blue line is the estimated regression, obtained by setting the intercept and slope to their posterior means, surrounded by its 95% credible interval."}
x <- data$x
y <- data$y
...
In `brms`, you just do: 
```{r ppcheck-brms, echo = TRUE, fig.cap = "Posterior predictive checks produced with brms. The black curve corresponds to the observed data; the blue curves to data simulated under the model. The x-axis shows the possible values of the simulated or observed response. The y-axis shows their estimated density.", dpi = 600}
pp_check(lm.brms)
```
...
```{r ppcheck-nimble, echo = TRUE, fig.cap = "Posterior predictive checks produced with NIMBLE. The black curve corresponds to the observed data; the blue curves to data simulated under the model. The x-axis shows the possible values of the simulated or observed response. The y-axis shows their estimated density."}
ggplot() +
...
```{r include = FALSE}
beta0 <- 0.1 # intercept true value
beta1 <- 1 # coefficient true value
sigma <- 0.5 # standard dev
set.seed(666) # 
n <- 100 # sample size
x <- rnorm(n = n, mean = 0, sd = 1) # simulate covariate
