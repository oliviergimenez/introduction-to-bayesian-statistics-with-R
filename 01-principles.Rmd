...
```{r ragondinos, fig.cap = "Photograph of coypus (Myocastor coypus) taken in the Lez watershed near Montpellier, France. Credits: Yann Raulet.", echo = FALSE, dpi = 600}
...
```{r bernoulli-binomiale, echo=FALSE, fig.cap="Discrete probability distributions, Bernoulli and binomial, illustrated with 100 simulations (random draws generated by computer). On the top row, we show the observed frequency from a Bernoulli draw for different values of survival probability \\(\\theta\\). On the bottom row, we show histograms for a binomial draw with 50 trials and different values of survival probability \\(\\theta\\).", fig.width=12, fig.height=6, message=FALSE, warning=FALSE, dpi = 600}
...
# --- Binomial N = 50, theta = 0.5 ---
bin1 <- rbinom(100,50,0.5) %>%
  as_tibble() %>%
  count(value) %>%
  ggplot(aes(x = value, y = n)) +
  geom_col(fill = "#E69F00", color = "black") +
  labs(title = expression("Binomial with n = 50 and " * theta * " = 0.5"), x = "number of survivors", y = NULL) 

# --- Binomial N = 50, theta = 0.2 ---
bin2 <- rbinom(100,50,0.2) %>%
  as_tibble() %>%
  count(value) %>%
  ggplot(aes(x = value, y = n)) +
  geom_col(fill = "#E69F00", color = "black") +
  labs(title = expression("Binomial with n = 50 and " * theta * " = 0.2"), x = "number of survivors", y = NULL) 

# --- Binomial N = 50, theta = 0.8 ---
bin3 <- rbinom(100,50,0.8) %>%
  as_tibble() %>%
  count(value) %>%
  ggplot(aes(x = value, y = n)) +
  geom_col(fill = "#E69F00", color = "black") +
  labs(title = expression("Binomial with n = 50 and " * theta * " = 0.8"), x = "number of survivors", y = NULL) 
...
```{r survie-vraisemblance-mle, echo = FALSE, fig.cap = "Likelihood function for the winter survival probability of the coypu, computed from $y=19$ survivors out of $n=57$ individuals monitored by GPS. The maximum likelihood estimate is indicated by the red dashed line.", dpi = 600}
y <- 19
n <- 57
grid <- seq(0, 1, 0.01)
likelihood <- dbinom(y, n, grid)
theta_mle <- y / n
...
df %>%
  ...
  labs(x = expression("Survival probability (" * theta * ")"),
       y = "likelihood") +
  theme_minimal()
```
...
```{r beta-exemples, echo = FALSE, fig.cap = "Examples of beta distributions for different values of parameters $a$ and $b$. In each panel, the shaded areas illustrate the probability of observing a value within a given interval.", dpi = 600}
...
```{r posterior-survie, echo = FALSE, fig.cap = "Uniform prior (red) and posterior distribution (black) of the coypu winter survival probability. The blue dashed line corresponds to the maximum likelihood estimate.", dpi = 600}
df_post <- data.frame(
  theta = x,
  prior = dbeta(x, 1, 1),
...
df_post %>%
  ...
  labs(x = expression("Survival probability (" * theta * ")"), y = "Density") +
  theme_minimal()
```
